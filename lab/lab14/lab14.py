...
def num_splits(s, d):
    ...
    def hepler(lst, prev):
        if len(lst) == 0:
            if abs(sum_s - 2 * (sum(lst) + prev)) <= d:return 1
            else: return 0
        return hepler(lst[1:], prev) + hepler(lst[1:], prev + lst[0])
    return hepler(s,0) // 2
...
class CheckingAccount(Account):
    ...
    def deposit_check(self, check):
        if check.payable_to != self.holder or check.deposited is True:
            print("The police have been notified.")
            return
        self.deposit(check.balance)
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        edit, cost = "-[" + skel_char + "]" + possibility[0], possibility[1] + 1
        possibilities.append((edit, cost))
