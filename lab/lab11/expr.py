...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        #if type(func) == PrimitiveFunction:
        #    return PrimitiveFunction.apply(func,args)
        #elif type(func) == LambdaFunction:
        #    return LambdaFunction.apply(func,args)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        "*** YOUR CODE HERE ***"
        if len(self.parameters) != len(arguments):
            raise ValueError("Invalid arguments {} to {}".format(
                comma_separated(arguments), self.parameters))
        p_copy = self.parent.copy()
        update = dict(zip(self.parameters,arguments))
        p_copy.update(update)
        return self.body.eval(p_copy)
